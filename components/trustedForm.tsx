...
function App() {
  ...
  const [currentStep, setCurrentStep] = useState(1); // Assuming 1 is 'ABOUT YOU'
  ...
  useEffect(() => {
    try {
      ...
      if (Object.keys(firebaseConfig).length === 0) {
        console.error("Firebase config is not provided. Backend functionality will be limited.");
  ...
  const handleSubmit = async () => {
    if (!email || !agreeToPrivacy) {
      setMessage('Please enter your business email and agree to the privacy policy.');
      return;
    }

    if (!db || !userId) {
      setMessage('Firebase is not initialized or user not authenticated. Cannot save data.');
    ...
    try {
      ...
      const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
      const userDocRef = doc(collection(db, `artifacts/${appId}/users/${userId}/proposals`));
      ...
      if (result.candidates && result.candidates.length > 0 &&
        ...
        setMessage('Proposal request submitted successfully, but failed to generate a confirmation message.');
        console.error('Failed to get a valid response from the generative model.');
      }
      ...
      setMessage(`Failed to submit proposal: ${error instanceof Error ? error.message : String(error)}`);
  ...
  const StepIndicator = ({ stepNumber, label, isActive }: { stepNumber: number; label: string; isActive: boolean }) => (
    <div className="flex flex-col items-center">
      ...
      <div className={`mt-2 text-xs font-medium ${isActive ? 'text-blue-600' : 'text-gray-500'}`}>
        {label}
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-transparent flex items-center justify-center p-4 font-inter">
      <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8 md:p-10 max-w-md w-full">
        ...
        <div className="flex justify-between items-center relative mb-8 px-4">
          ...
          <StepIndicator stepNumber={1} label="ABOUT YOU" isActive={currentStep === 1} />
          <StepIndicator stepNumber={2} label="YOUR NEEDS" isActive={currentStep === 2} />
          <StepIndicator stepNumber={3} label="BOOKING" isActive={currentStep === 3} />
        </div>
        ...
        <div className="flex items-center mb-8">
          ...
          <label htmlFor="privacyPolicy" className="ml-2 text-gray-600 text-sm">
            I agree to the <a href="#" className="text-black hover:underline" onClick={(e) => e.preventDefault()}>Privacy Policy</a>
          </label>
        </div>
