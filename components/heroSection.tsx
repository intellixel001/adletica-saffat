...
const HeroSection: React.FC = () => {
  const logos: Logo[] = [
    { src: "https://placehold.co/80x32/E0E0E0/333?text=Monopoly", alt: "Monopoly" },
  ...
  // While the container is fixed, the image inside will be responsive with object-contain.
  ...
  const VolumeMuteIcon = ({ className }: { className?: string }) => (
    <svg className={className} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
      <path fillRule="evenodd" d="M9.383 3.076A1 1 0 0110 4v12a1 1 0 01-1.707.707L4.586 13H2a1 1 0 01-1-1V8a1 1 0 011-1h2.586l3.707-3.707a1 1 0 011.09-.217zM14.657 6.343a1 1 0 010 1.414L13.536 9.5a1 1 0 01-1.414-1.414l1.121-1.121a1 1 0 011.414 0zm2.828 2.828a1 1 0 010 1.414L16.364 12.5a1 1 0 01-1.414-1.414l1.121-1.121a1 1 0 011.414 0z" clipRule="evenodd"></path>
    </svg>
  );
    ...
    <section className="w-full bg-white px-4 md:px-8 lg:px-16 pt-30 font-sans">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
        ...
        <div className="space-y-6">
          ...
          <Link href="/hire" className="inline-flex items-center px-6 py-3 border-blue-600 border-2 bg-white text-blue-600 rounded-full hover:bg-blue-600 hover:text-white transition-all duration-300 shadow-lg text-4xl sm:text-lg cursor-pointer">
            Book a call <ArrowRightIcon className="ml-2 w-5 h-5" />
          </Link>

          {/* Logos Carousel with Framer Motion */}
            ...
            >
              {duplicatedLogos.map((logo: Logo, index: number) => (
                <div key={index} className="flex-none" style={{ width: itemWidth + 'px' }}>
                  {/* Image tag with Tailwind classes for responsive sizing and centering.
                      `object-contain` ensures logos scale down without cropping on narrow screens. */}
                  <img src={logo.src} alt={logo.alt} className="mx-auto w-20 h-8 object-contain" />
                </div>
              ))}
            </motion.div>
          </div>
          ...
          <div className="mt-4 text-sm sm:text-base text-gray-600">
            Reviewed on <span className="font-bold text-black">Clutch</span> – 5 STAR AGENCY
          </div>
        </div>

        {/* Background Video - Top Right side on larger screens.
            `w-full` and `h-96` (a fixed height that works well for most mobile views)
            combined with `object-cover` ensure the video adapts to the container. */}
        <div className="relative w-full h-96 md:h-[500px] lg:h-[600px] rounded-3xl overflow-hidden shadow-xl">
          <video
            src="https://www.w3schools.com/html/mov_bbb.mp4" // Using a working video URL for demo
            loop
            autoPlay
            muted={isMuted}
          ...
          {/* Mute/Unmute button for the video */}
          <button
          ...
          >
            {isMuted ? (
              <VolumeMuteIcon className="h-6 w-6" /> // Mute icon
            ) : (
              <VolumeUpIcon className="h-6 w-6" /> // Unmute icon
            )}
          </button>
        </div>
      </div>

      {/* Bottom CTA Section - Similar responsive layout to the top section */}
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 items-center mt-16 py-10">
        {/* Background Video - Bottom Left side on larger screens */}
        ...
        {/* Text Content - Bottom Right side on larger screens */}
        <div className="space-y-6">
          ...
          <p className="text-lg sm:text-xl text-gray-700">
            Embed a hands-on team of Growth Marketing experts in your business as your full-stack growth marketing
            department, or as an extension of your internal marketing team.
          </p>
          {/* Call to action button */}
          <Link href="/hire" className="inline-flex items-center px-6 py-3 border-blue-600 border-2 bg-white text-blue-600 rounded-full hover:bg-blue-600 hover:text-white transition-all duration-300 shadow-lg text-4xl sm:text-lg cursor-pointer">
